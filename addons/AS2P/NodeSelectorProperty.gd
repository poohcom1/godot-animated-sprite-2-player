...
func get_animatedsprite() -> AnimatedSprite2D:
	...
	var animated_sprites_nodes = _get_animated_sprites(root)
	if drop_down.selected < 0 or drop_down.selected >= animated_sprites_nodes.size():
		return null

	return animated_sprites_nodes[drop_down.selected]
...
func convert_sprites():
	var animated_sprite: AnimatedSprite2D = get_animatedsprite()

	if not animated_sprite:
		print("[AS2P] No AnimatedSprite selected or scene is empty!")
		return
